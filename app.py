...
st.write("Arraste o arquivo CSV do sistema para gerar o relatório com os IDs, Atendentes e Motivos.")
...
if uploaded_file is not None:
    ...
    if 'feedback_score' in df.columns:
        ...
        bad_ratings['Atendente'] = bad_ratings['agent_login'].apply(
            lambda x: str(x).split('@')[0] if pd.notna(x) else 'Sem Login (Erro de Sistema)'
        )
        bad_ratings['Motivo (Tabulação)'] = bad_ratings['tabulation'].fillna('Em branco (Erro do Atendente)')
        
        # Selecionando e renomeando as colunas que importam
        final_df = bad_ratings[['id', 'Atendente', 'feedback_score', 'Motivo (Tabulação)', 'created_at']]
        final_df.columns = ['ID do Atendimento', 'Atendente', 'Nota', 'Motivo', 'Data do Atendimento']
        ...
        with col1:
            st.write("**Gargalos por Motivo:**")
            resumo_motivo = final_df.groupby('Motivo').size().reset_index(name='Quantidade')
